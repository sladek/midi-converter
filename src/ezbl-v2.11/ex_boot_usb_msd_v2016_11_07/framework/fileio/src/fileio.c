...
FILEIO_TimestampGet timestampGet;
    ...
    uint8_t __attribute__ ((aligned(4)))    gDataBuffer[FILEIO_CONFIG_MAX_DRIVES][FILEIO_CONFIG_MEDIA_SECTOR_SIZE];     // The global data sector buffer(s)
    uint8_t __attribute__ ((aligned(4)))    gFATBuffer[FILEIO_CONFIG_MAX_DRIVES][FILEIO_CONFIG_MEDIA_SECTOR_SIZE];      // The global FAT sector buffer(s)
#else
...
int FILEIO_Initialize (void)
{
    ...
    FILEIO_TestStructPacking(); // Compile time tests to see if structures have been defined correctly. This does not generate any program code.

    for (i = 0; i < FILEIO_CONFIG_MAX_DRIVES; i++)
    {
        gDriveSlotOpen[i] = true;
        gDriveArray[i].dataBuffer = gDataBuffer[i];
        gDriveArray[i].fatBuffer = gFATBuffer[i];
        gDriveArray[i].bufferStatusPtr = &bufferStatus[i];
        bufferStatus[i].driveOwner = NULL;
...
FILEIO_ERROR_TYPE FILEIO_DriveMount (char driveId, const FILEIO_DRIVE_CONFIG * driveConfig, void * mediaParameters)
{
    ...
    drive->bufferStatusPtr->flags.fatBufferNeedsWrite = false;
...
/**
 * Reads the Master Boot Record from the specified drive and checks it for the
 * presence of a FAT12/FAT16/FAT32 file system on any of the partitions.
 *
 * @return FILEIO_ERROR_NONE - Found a FAT12/FAT16/FAT32 volume
...
FILEIO_ERROR_TYPE FILEIO_LoadMBR (FILEIO_DRIVE * drive)
{
    ...
    uint8_t * rdPtr;
    uint_fast8_t i;
    FILEIO_MBR_PARTITION_TABLE_ENTRY partition;
    ...
    // Check if the media has no MBR
    ptrBootSector = (FILEIO_BOOT_SECTOR *)drive->dataBuffer;

    if ((drive->dataBuffer[FILEIO_FAT_GOOD_SIGN_0_OFFSET] == FILEIO_FAT_GOOD_SIGN_0) && (drive->dataBuffer[FILEIO_FAT_GOOD_SIGN_1_OFFSET] == FILEIO_FAT_GOOD_SIGN_1))
    {
     ...
     // memory.  See the PCMCIA metaformat for more details
        ...
        if ((*(drive->dataBuffer + BSI_FAT32_FSTYPE + 0 ) == 'F') && \
    ...
    // Ensure its good
    if((drive->dataBuffer[FILEIO_MBR_SIGNATURE0_OFFSET] != FILEIO_FAT_GOOD_SIGN_0) || (drive->dataBuffer[FILEIO_MBR_SIGNATURE1_OFFSET] != FILEIO_FAT_GOOD_SIGN_1))
        return FILEIO_ERROR_BAD_PARTITION;


    drive->type = FILEIO_FILE_SYSTEM_TYPE_NONE;
    rdPtr = drive->dataBuffer + FILEIO_MBR_PARTITION0_OFFSET;
    for(i = 0; i < 4u; i++)
    {
        // Valid Master Boot Record found; enumerate Partition Table 0-3 entries for a FAT volume by first copying to RAM aligned structure
        memcpy(&partition, rdPtr, sizeof(FILEIO_MBR_PARTITION_TABLE_ENTRY));

        // Get the 32 bit offset to the first partition
        drive->firstPartitionSector = partition.lbaFirstSector;

        // check if the partition type is acceptable
        switch (partition.fileSystemDescriptor)
        {
            case 0x01:
        ...
        rdPtr += sizeof(FILEIO_MBR_PARTITION_TABLE_ENTRY);
    }
...
FILEIO_ERROR_TYPE FILEIO_LoadBootSector (FILEIO_DRIVE * drive)
{
    ...
    ptrBootSector = (FILEIO_BOOT_SECTOR *)drive->dataBuffer;
    while(1)    //test each possible boot sector (FAT32 can have backup boot sectors)
    {
...
int FILEIO_DotEntryWrite (FILEIO_DRIVE * drive, uint32_t dot, uint32_t dotdot, FILEIO_TIMESTAMP * timeStamp)
{
    ...
    entryPtr->createDate   = timeStamp->time.value; // date created
...
int FILEIO_CreateMBR (FILEIO_DRIVE_CONFIG * config, void * mediaParameters, uint32_t firstSector, uint32_t sectorCount)
{
    ...
    if (!gDriveSlotOpen[FILEIO_CONFIG_MAX_DRIVES - 1])
    {
        if (bufferStatusPtr->flags.dataBufferNeedsWrite)
        {
            if (! (*config->funcSectorWrite)(mediaParameters, bufferStatusPtr->dataBufferCachedSector, dataBuffer, false))
            {
                return false;
            }
            bufferStatusPtr->flags.dataBufferNeedsWrite = false;
...
#else
uint16_t __attribute__((optimize(1))) ReadRam16bit (uint8_t * pBuffer, uint16_t index)
{
    pBuffer += index;
    return (((uint16_t)pBuffer[1])<<8) | (((uint16_t)pBuffer[0]));
}

uint32_t __attribute__((optimize(1))) ReadRam32bit (uint8_t * pBuffer, uint16_t index)
{
    union
    {
        uint32_t u32;
    ...
    pBuffer += index;
    ret.u16[0] = (((uint16_t)pBuffer[1])<<8) | (((uint16_t)pBuffer[0]));
    ret.u16[1] = (((uint16_t)pBuffer[3])<<8) | (((uint16_t)pBuffer[2]));
    return ret.u32;
}
#endif
