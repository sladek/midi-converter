...
#define __FILEIO_PACKED__   __attribute__((packed))
#else
#define __FILEIO_PACKED__
#endif
...
#define FILEIO_FAT_GOOD_SIGN_0_OFFSET   510
#define FILEIO_FAT_GOOD_SIGN_1_OFFSET   511
...
typedef struct {
    ...
    } __FILEIO_PACKED__;    // 24 bytes so far
    struct
    {
...
} FILEIO_BIOS_PARAMETER_BLOCK_FAT12;  // Must be 62 bytes
...
} FILEIO_BIOS_PARAMETER_BLOCK_FAT16;  // Must be 62 bytes

// BIOS Parameter Block for a FAT32 partition
typedef struct {
    struct
    {
        uint8_t jumpCommand[3];                 // Jump Command
    ...
    } __FILEIO_PACKED__;  // Must be 24 bytes, 8-bit packing required
    struct
    {
        uint16_t sectorsPerTrack;               // Number of sectors per track
        uint16_t headCount;                     // Number of heads
...
} FILEIO_BIOS_PARAMETER_BLOCK_FAT32;    // Must be 90 bytes total
...
} FILEIO_MBR_PARTITION_TABLE_ENTRY;   // Must be 16 bytes, no packing required

// Structure of a device's master boot record
typedef struct
{
...
} __FILEIO_PACKED__ FILEIO_MASTER_BOOT_RECORD;


#define FILEIO_MBR_BOOTCODE_OFFSET      0
#define FILEIO_MBR_BOOTCODE_LEN         446
#define FILEIO_MBR_PARTITION0_OFFSET    (446+(16*0))
#define FILEIO_MBR_PARTITION1_OFFSET    (446+(16*1))
#define FILEIO_MBR_PARTITION2_OFFSET    (446+(16*2))
#define FILEIO_MBR_PARTITION3_OFFSET    (446+(16*3))
#define FILEIO_MBR_SIGNATURE0_OFFSET    510         // MBR signature code byte 0 - should contain 0x55
#define FILEIO_MBR_SIGNATURE1_OFFSET    511         // MBR signature code byte 1 - should contain 0xAA
...
typedef struct
{
    ...
    union
    {
        FILEIO_BIOS_PARAMETER_BLOCK_FAT32 fat32;    // 90 bytes
        FILEIO_BIOS_PARAMETER_BLOCK_FAT16 fat16;    // 62 bytes
        FILEIO_BIOS_PARAMETER_BLOCK_FAT12 fat12;    // 62 bytes
    } biosParameterBlock;
...
} __FILEIO_PACKED__ FILEIO_BOOT_SECTOR;


#if defined(__XC8__)
#define FILEIO_TestStructPacking()  // Natural 8-bit packing is already correct on 8-bit machines
#else
static inline void __attribute__((always_inline, optimize(1))) FILEIO_TestStructPacking(void)   // Compile-time tests various structures to ensure they are the correct size. Applying packing to unnecessary elements generates much inferior code, but 16 and 32-bit architectures need packing to correspond to disk structures.
{
    if(sizeof(FILEIO_MBR_PARTITION_TABLE_ENTRY) != 16u)
        __asm__(".error \"FILEIO_MBR_PARTITION_TABLE_ENTRY, or something in it, needs to be packed\"");
    if(sizeof(FILEIO_MASTER_BOOT_RECORD) != 446u + 16u*4u + 2u)
        __asm__(".error \"FILEIO_MASTER_BOOT_RECORD, or something in it, needs to be packed\"");
    if(sizeof(FILEIO_BOOT_SECTOR) != 512u)
        __asm__(".error \"FILEIO_BOOT_SECTOR, or something in it, needs to be packed\"");
}
#endif
