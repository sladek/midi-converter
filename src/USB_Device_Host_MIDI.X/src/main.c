...
extern SCAN_STATE scan_state;
extern uint16_t usbHostState;
bool usb_host_attached = false;
...
//               LED1_Set_State(LED_STATUS_ON);
                ...
                while(scan_state == SCAN_STATE_HOST){

                    USBHostTasks();
                    if(usb_host_attached){
                        // Disable Timet2 as we don't need scanning anymore
                        disableTimer2();
                        // to make sure that it was not redefined by the timer
                        scan_state = SCAN_STATE_HOST;
                        // And continue as a host
                        while(1)
                        {
                            if(USBHostDeviceStatus(0) == STATE_DETACHED){
//                                asm("RESET");
                            }
                            USBHostTasks();
                            //Application specific tasks
...
bool USB_Host_ApplicationEventHandler( uint8_t address, USB_EVENT event, void *data, uint32_t size )
{
    switch( (int)event )
    {
        ...
        case EVENT_TRANSFER:
             return APP_HostMIDIBasicEventHandler( address, event, data, size );            
        /* MIDI Class Specific Events ******************************************/
        case EVENT_MIDI_ATTACH:
             LED2_Set_State(LED_STATUS_OFF);
             usb_host_attached = true;
             return APP_HostMIDIBasicEventHandler( address, event, data, size );
